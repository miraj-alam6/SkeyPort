...

{
	...
	for(i=0;i<4;i++)
		for(j=0;j<32;j=j+8)
			printf("%02u",(MDp->buffer[i]>>j) & 0xFF);  //changed this from lx to u
}
...
{
  DEBUG_ONENTER
  fprintf(logfile, "Entering function %s(MDp = \"%p\")\n",__func__, MDp);
	...
	MDp->done = 0;
  DEBUG_ONEXIT
  fprintf(logfile, "Exiting function %s returnvalue is(s = \"%s\")\n",__func__, "NO RETURN VALUE FOR VOID FUNCTION");
...
{
  DEBUG_ONENTER
  fprintf(logfile, "Entering function %s(X = %p)\n",__func__, X);
 ...
 */
static void MDblock(MDptr MDp,unsigned int *X) //changed this from long to int
{
  DEBUG_ONENTER
  fprintf(logfile, "Entering function %s(MDp = \"%p\", X = %p)\n",__func__, MDp,X);

	register unsigned int tmp, A, B, C, D; //changed this from long to in
...
{
  DEBUG_ONENTER
  fprintf(logfile, "Entering function %s(MDp = \"%p\", X = %s, count = %u )\n",__func__, MDp,X,count);
	int i,bit,byte,mask;
	unsigned long tmp;
	unsigned char XX[64];
	unsigned char *p;
	...
	if(count == 512){
		...
		MDblock(MDp,(unsigned int *)X);  //changed to from long to int
	} else if(count > 512){
		/* Check for count too large */
	  printf("\nError: MDupdate called with illegal count value %d.",count); //#MIRAJ I added the casting
		return;
	} else {
		...
		if(byte <= 55){
			...
			MDblock(MDp,(unsigned int *)XX); //Changed this from long to int
			for(i=0;i<56;i++)
				XX[i] = 0;
			for(i=0;i<8;i++)
				XX[56+i] = MDp->count[i];
