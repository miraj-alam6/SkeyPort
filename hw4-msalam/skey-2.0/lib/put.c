 ...
 */
char * btoe (char *engout, char *c)  // #NOTE #MIRAJ  in Pre-Ansi when it said the type, it mis-matched the order when it said the types
{                                   //not sure if that matters, but keep note of it.
...
static void standard (register char *word)
{
  while (*word)
  {
    if (!isascii (*word))
      break;
    if (islower ((int)*word)) // # MIRAJ i made this change
      *word = toupper(*word); // #ASK when do we need to make it unsigned char? Above line gave me error but shouldn't it matter more for upper
    if (*word == '1')
      *word = 'L';
    if (*word == '0')
      *word = 'O';
